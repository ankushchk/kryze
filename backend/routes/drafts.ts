...
export function parseSms(body: string): { merchant: string; amount: number } {
  ...
  // Try to find merchant name in the SMS body. Usually it follows words like "at", "to", "vpa", "info", "for"
  ...
  for (const regex of merchantRegexes) {
    ...
    if (match && match[1]) {
      ...
      // Clean up common VPA suffix or generic words
      if (candidate.includes("@")) {
        candidate = candidate.split("@")[0];
      }
      
      // Strip out card numbers or account references like "a/c xx123"
      candidate = candidate.replace(/a\/c\s*x*/gi, "");
      candidate = candidate.replace(/xx[0-9]*/gi, "");
      candidate = candidate.replace(/acct\s*x*/gi, "");
      
      // Clean trailing non-alphanumeric characters (like punctuation or spaces)
...
// POST /api/drafts (Create or update a single finalized/ignored transaction)
